            ...
            if (isChatGroup === "true") {
                ...
                if (messageVal.length > 0) {
                    let arrayFilesPromise = messageVal.map(async fileVal => {
                        let messageType = MessageModel.messageTypes.ATTACHMENT;
                        ...
                        if (contentType === videoType[0]) {
                            messageType = MessageModel.messageTypes.VIDEO;
                        }
                    ...
                    newMessageItem = {
                        ...
                        userCurrent: getChatGroupReceiver.members,
                        conversationType: MessageModel.conversationTypes.GROUP,
                        file: arrayFilesMess.length ? arrayFilesMess : [],
                    };
                }
                ...
                if (messageVal.length > 0) {
                    ...
                    newMessageItem = {
                        ...
                        userCurrent: [senderId, receivedId],
                        conversationType: MessageModel.conversationTypes.PERSONAL,
                        file: arrayFilesMess.length > 0 ? arrayFilesMess : [],
                    };
                }
                //create new message
                let newMessage = await (await MessageModel.model.create(newMessageItem)).populate(
...
let deleteAllMessagesInPersonal = (currentUserId, userChatId) => {
    return new Promise(async (resolve, reject) => {
        try {
            let allMessages = await MessageModel.model.getAllMessage(currentUserId, userChatId);
            ...
            if (deleteAllMessages.n === 0) {
                ...
                let arrayFile = [];
                allMessages.map(mes => {
                    arrayFile = arrayFile.concat(mes.file)
                })
                await Promise.all(
                    arrayFile.map(async(f) => {
