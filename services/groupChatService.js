...
let addNewGroup = (currentUserId, arrayMemberIds, groupChatName) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            let abc = [];
            arrayMemberIds.map(user => {
                abc.push(user.userId);
            })
            ...
            let usersAddedInfo = await UserModel.getNormalUsersDataByIdsAndLimit(usersAddedId,20);
            newGroup = Object.assign(newGroup._doc, {
                messages: [],
                membersInfo: usersAddedInfo
            });
